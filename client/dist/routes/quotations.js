...
router.get('/', async (req, res) => {
  ...
  const rows = await db.all(`SELECT q.*, c.full_name as customer_name FROM quotations q LEFT JOIN customers c ON q.customer_id = c.id WHERE ${where} ORDER BY q.created_at DESC LIMIT ? OFFSET ?`, [...params, parseInt(limit, 10), offset]);
...
router.delete('/:id', async (req, res) => {
  const db = await getDb();
  await moveToTrash(db, 'quotations', req.params.id, req.user?.id);
