...
router.get('/', async (req, res) => {
  ...
  const rows = await db.all(`SELECT * FROM expenses WHERE ${where} ORDER BY created_at DESC LIMIT ? OFFSET ?`, [...params, parseInt(limit, 10), offset]);
  ...
  res.json({ rows, total: count.count, page: parseInt(page, 10), sum: total.total, categories: categories.map(c => c.category) });
});
...
router.put('/:id', async (req, res) => {
  ...
  await db.run(`UPDATE expenses SET description=?, amount=?, category=?, expense_date=?, payment_method=?, notes=? WHERE id=?`,
    [description, amount, category, expense_date, payment_method, notes, req.params.id]);
  res.json({ success: true });
});

router.delete('/:id', async (req, res) => {
  const db = await getDb();
  await moveToTrash(db, 'expenses', req.params.id, req.user?.id);
