...
router.get('/', async (req, res) => {
  ...
  if (is_published !== undefined) {
    ...
    params.push(parseInt(is_published, 10));
  }
  const count = await db.get(`SELECT COUNT(*) as count FROM knowledge_articles WHERE ${where}`, params);
  const rows = await db.all(`SELECT * FROM knowledge_articles WHERE ${where} ORDER BY created_at DESC LIMIT ? OFFSET ?`, [...params, parseInt(limit, 10), offset]);
