...
router.get('/', async (req, res) => {
  ...
  const rows = await db.all(`SELECT b.*, c.full_name as customer_name, (SELECT COUNT(*) FROM booking_services WHERE booking_id = b.id) as service_count, (SELECT COUNT(*) FROM booking_passengers WHERE booking_id = b.id) as passenger_count FROM bookings b LEFT JOIN customers c ON b.customer_id = c.id WHERE ${where} ORDER BY b.created_at DESC LIMIT ? OFFSET ?`, [...params, parseInt(limit, 10), offset]);
