...
router.get('/', async (req, res) => {
  ...
  const rows = await db.all(`SELECT * FROM tour_packages WHERE ${where} ORDER BY created_at DESC LIMIT ? OFFSET ?`, [...params, parseInt(limit, 10), offset]);
...
router.delete('/:id', async (req, res) => {
  const db = await getDb();
  await moveToTrash(db, 'tour_packages', req.params.id, req.user?.id);
